...
const REPO = 'antdv-next/antdv-next-pro'
...
const importCode = computed(() =>
  IMPORT_OVERRIDES[slug.value] ?? `import { ${component.value} } from '@antdv-next/pro'`)

const sourceRepo = computed(() => SOURCE_OVERRIDES[slug.value]?.repo ?? REPO)
const sourcePath = computed(() => {
  const override = SOURCE_OVERRIDES[slug.value]
  if (override) {
    return override.repo ? override.dir : `packages/pro/src/${override.dir}`
  }
  return `packages/pro/src/${slug.value}`
})
