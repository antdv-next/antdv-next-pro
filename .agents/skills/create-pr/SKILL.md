---
name: create-pr
description: Create or prepare pull requests for antdv-next-pro using this repository's official PR templates. Use this skill when the user says “PR” or “提交 PR”, asks to create/open/submit a PR, requests a PR title or body, or wants branch changes summarized for a PR. Trigger by PR-creation intent, including short colloquial requests; do not trigger for purely conceptual discussions about pull requests.
---

# antdv-next-pro PR 创建规范

## 目标

一、基于当前分支相对基线分支的全部已提交改动生成 PR，不只看最后一个 commit。

二、严格使用 `antdv-next-pro` 仓库自带的中英文模板，不自行发明 PR 结构。
...
不要修改 section 名称或删除主结构。填写时可删除 HTML 注释、引用式说明和占位文本，但必须保留最终提交所需的类型、Related Issues、Background and Solution、Change Log、Component Contribution Checklist 等 section。
...
```bash
gh pr view --repo antdv-next/antdv-next-pro --json url,title,baseRefName,headRefName,state
```
...
必要时查看相关测试、文档和配置文件。摘要必须覆盖整条分支，不得只复述最后一个 commit 或逐文件罗列。
...
- 组件新能力、公开 API 或新增组件：`feat` / `🆕 New feature` 或 `🧩 New component`
- 现有组件功能增强：`feat` / `⭐️ Existing component enhancement`
...
- Component Contribution Checklist：涉及组件文档、demo、测试或兼容性时，如实勾选已完成项。
...
- 使用英文概括整条分支的主要结果；
- 单一组件或明确模块才使用 scope，如 `fix(Scrollbar): keep thumb stable on content resize`；
...
- Target repository：应为 `antdv-next/antdv-next-pro`；
...
git branch -vv
git remote -v
gh repo view --repo antdv-next/antdv-next-pro --json nameWithOwner,defaultBranchRef
```

要求：

1. 目标仓库必须明确为 `antdv-next/antdv-next-pro`，不要依赖 `gh` 的当前仓库推断。
...
```bash
gh pr create \
  --repo antdv-next/antdv-next-pro \
  --base <base> \
  --head <owner>:<branch> \
  --title "<title>" \
  --body-file <body-file>
