...
const packageUrl = fileURLToPath(new URL('../../', import.meta.url))
...
async function run() {
  ...
  await fs.writeFile(path.resolve(packageUrl, CONFIG.outputWebTypes), `${JSON.stringify(webTypes, null, 2)}\n`, 'utf8')
  ...
  await fs.writeFile(path.resolve(packageUrl, CONFIG.outputVSCode), `${JSON.stringify(vscodeData, null, 2)}\n`, 'utf8')
